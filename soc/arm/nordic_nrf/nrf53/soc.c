...
static bool rtc_ticks_to_next_event_get(NRF_RTC_Type * rtc, uint32_t selected_cc_mask, uint32_t cntr, uint32_t * ticks_to_next_event)
{
	bool result = false;

	/* Let's preload registers to speed-up. The function is not thread-safe anyway. */
	uint32_t reg_intenset = rtc->INTENSET;

	/* If corresponding EVTEN bit is zero, then interrupt won't be generated as the event itself wont be generated. */
	reg_intenset &= rtc->EVTEN;
	...
	for (uint32_t chan = 0; chan < NRF_RTC_CC_COUNT_MAX; chan++) {
		if ((selected_cc_mask & (1U << chan)) &&
			...
			if (ticks_to_fire == 0U) {
				/* When ticks_to_fire == 0, the event should have been just generated
				 * the interrupt can be already handled or be pending.
...
static void rtc_counter_synchronized_get(NRF_RTC_Type * rtc_a, NRF_RTC_Type * rtc_b, uint32_t * counter_a, uint32_t * counter_b)
{
	do {
...
bool z_arm_on_enter_cpu_idle(void)
{
	...
	if (ok_to_sleep) {
		...
		if (!nrf_rtc_event_check(NRF_RTC1, NRF_RTC_CHANNEL_EVENT_ADDR(RTC1_PRETICK_CC_CHAN))) {
